...
+ (YQURLSessionTask *)postWithUrl:(NSString *)url
                   refreshRequest:(BOOL)refresh
                    ...
                    progressBlock:(YQPostProgress)progressBlock
                     successBlock:(YQResponseSuccessBlock)successBlock
                        failBlock:(YQResponseFailBlock)failBlock {
    NSLog(@"%@?%@",url,[self getHttpBodyStringWithParams:params]);
    __block YQURLSessionTask *session = nil;
...
#pragma mark - 传一个字典拼接字符串
+ (NSString *)getHttpBodyStringWithParams:(NSDictionary *)params
{
    if (params.count) {
        ...
        for (NSString *key in params) {
            ...
            if ([value isKindOfClass:[NSArray class]]) {
                ...
                for (int i = 0; i < valueArray.count; i++) {
                    NSString *vlue = [NSString stringWithFormat:@"%@",valueArray[i]];
                    httpBody = [httpBody stringByAppendingString:[NSString stringWithFormat:@"%@=%@&",[self UrlEncodedString:key],[self UrlEncodedString:vlue]]];
                }
            } else {
                NSString *vlue = [NSString stringWithFormat:@"%@",params[key]];
                ///处理中文字符和特殊字符的情况
                httpBody = [httpBody stringByAppendingString:[NSString stringWithFormat:@"%@=%@&",[self UrlEncodedString:key],[self UrlEncodedString:vlue]]];
            }
        }
